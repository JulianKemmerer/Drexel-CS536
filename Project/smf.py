...
class SMFModel:
	...
	#Return max and min z values from this model
	#return maxz,minz
	def get_max_min_z(self):
		minz = 999
		maxz = -999
		for vertex in self.vertices:
			z = vertex.zbuff_copy.z
			if z > maxz:
				maxz = z
			if z < minz:
				minz = z
	...
	#Populate from file
	def build_from_file(self,filename,xpm_color = None):
		#Set color
		self.xpm_color = xpm_color
...
def get_smf_models(smf_files,xpm_colors=None):
	all_models = []
	xpm_index = 0
	for smf_file in smf_files:
		color = None
		if not(xpm_colors is None):
			color = xpm_colors[xpm_index]
			xpm_index = xpm_index + 1
		model = get_smf_model(smf_file,color)
		all_models = all_models + [model]
	return all_models		
	
#Get smf model from single file
def get_smf_model(smf_file,xpm_color = None):
	model = SMFModel()
	model.build_from_file(smf_file,xpm_color)
	return model

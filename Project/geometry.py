...
class Point(GeometricObject):
	...
	#Z buffered version of this vertex
	zbuff_copy = None
...
class Polygon(GeometricObject):
	...
	#Store max and min z values from parent smf file
	parent_smf_max_z = None
	parent_smf_min_z = None
	...
	def __init__(self):
		...
		self.xpm_color = None
		#Store max and min z values from parent smf file
		self.parent_smf_max_z = None
		self.parent_smf_min_z = None
		
	#Given an x,y value get z
	def get_z_at_xy(self,x,y):
		#Get 3 points for this plane (use copied z buff version)
		p1 = self.vertices[0].zbuff_copy
		p2 = self.vertices[1].zbuff_copy
		p3 = self.vertices[2].zbuff_copy
		...
		p1_p2.y = p1.y - p2.y
		p1_p2.z = p1.z - p2.z
		p1_p2_v = p1_p2.to_vector3d()
		#p2->p3
		p2_p3 = Point()
		p2_p3.x = p2.x - p3.x
		p2_p3.y = p2.y - p3.y
		p2_p3.z = p2.z - p3.z
		p2_p3_v = p2_p3.to_vector3d()
		
		#Cross the vectors
		normal = numpy.cross(p1_p2_v,p2_p3_v)
		a = normal[0]
		b = normal[1]
		c = normal[2]
	...
	#Given x,y get xpm fp color value
	def get_xpm_fp_color_at_xy(self,x,y):
		#First get z value here
		z = self.get_z_at_xy(x,y)
		
		#Use max and min from poly, which was given from parent smf
		zmax = float(self.parent_smf_max_z)
		zmin = float(self.parent_smf_min_z)
		percent = (z-zmin)/(zmax-zmin)
		
		#Use percent to pick color
		fp = xpm.RGB_MAX_VAL * percent
		
		#fp should not be above 255
		if fp > 255:
			fp = 255.0
		
		#Then use fp offset
		offset = None
		if self.xpm_color == xpm.COLOR_RED:
			offset = xpm.R_OFFSET
		elif self.xpm_color == xpm.COLOR_GREEN:
			offset = xpm.G_OFFSET
		elif self.xpm_color == xpm.COLOR_BLUE:
			offset = xpm.B_OFFSET
		else:
			offset = xpm.BACKGROUND_FP_VAL
		
		#Add fp value to offset for final fp value
		fp_val = fp + offset
...
def geometric_objects_to_points(objects,do_filling=False):
	#Loop over all geometric objects and collect list of points
	rv = dict()
	for i in range(0,len(objects)):
		obj = objects[i]
		rv[i] = obj.render_points(do_filling)
	return rv
...
def perform_projection(smf_models,use_parallel_projection, prp_z, B):
	...
	if use_parallel_projection:
		...
		#Do loop over and make z buffered copies
		#Loop over all models
		for smf_model in smf_models:
			#Loop over all vertices
			for vertex in smf_model.vertices:
				vertex.zbuff_copy = Point()
				vertex.zbuff_copy.x = vertex.x
				vertex.zbuff_copy.y = vertex.y
				vertex.zbuff_copy.z = vertex.z
	else:
		...
		for smf_model in smf_models:
			...
			for vertex in smf_model.vertices:			
				...
				#Before modifying z, save a copy in this vertex with z
				#valuie preserved in a xopy
				vertex.zbuff_copy = Point()
				vertex.zbuff_copy.x = vertex.x
				vertex.zbuff_copy.y = vertex.y
				vertex.zbuff_copy.z = vertex.z
				#Then change this vertex z
				vertex.z = d
...
def render_smf_to_polys(smf_models):
	...
	for smf_model in smf_models:
		...
		#Get max and min z values from smf model
		maxz,minz = smf_model.get_max_min_z()
		#Set color of these polys to be color of parent model
		#Also set parent max min Z
		for poly in polys:
			poly.xpm_color = smf_model.xpm_color
			poly.parent_smf_max_z = maxz
			poly.parent_smf_min_z = minz
		#Add to all polys list
...
def do_normalized_window_to_viewport_transform(smf_models, prp_z, B, vp_lower_bound_x, vp_lower_bound_y, vp_upper_bound_x, vp_upper_bound_y, use_parallel_projection):
	...
	for smf_model in smf_models:
		...
		for vertex in smf_model.vertices:
			...
			#Also apply to z buffered copy
			vertex.zbuff_copy.apply_transform(transform_matrix)
